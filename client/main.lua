if rx.UsePujcovnaCayo then
    ...
    AddEventHandler('rx_carrent:client:SpawnCar', function(data)
        ...
        if count >= rx.Ceny.Dingy.price then
            if ESX.Game.IsSpawnPointClear(vector3(4760.8804, -4784.9414, 0.3564), 3) then
                if data.car == "dinghy3" then
                        local player = PlayerPedId()
                        ESX.Game.SpawnVehicle('dinghy3', vector3(4760.8804, -4784.9414, 0.3564), 53.4376, function(vehicle)
                ...
                elseif data.car == "seashark" then
                    if count >= rx.Ceny.Skutr.price then
                        local player = PlayerPedId()
                        ESX.Game.SpawnVehicle('seashark', vector3(4760.8804, -4784.9414, 0.3564), 53.4376, function(vehicle)
                        if rx.TeleportToCar then 
                            TaskWarpPedIntoVehicle(player, vehicle, -1)
                        end
                        Entity(vehicle).state.fuel = 100
